...
# ── Sidebar: credentials and how it works ─────────────────────────────────────
with st.sidebar:
    st.header("🔑 Azure OpenAI")
    endpoint    = st.text_input("Endpoint",              placeholder="https://xxx.openai.azure.com/")
    api_key     = st.text_input("API Key",               type="password")
    chat_model  = st.text_input("Chat deployment",       value="gpt-4o")
    embed_model = st.text_input("Embeddings deployment", value="text-embedding-ada-002")

    st.divider()
    st.markdown("### How it works")
    st.markdown("1. **Load** — extract text from the PDF")
    st.markdown("2. **Split** — divide into 400-word chunks")
    st.markdown("3. **Embed** — turn each chunk into a vector")
    st.markdown("4. **Search** — find chunks closest to your question")
    st.markdown("5. **Answer** — GPT reads those chunks and answers")

    st.divider()
    if st.button("🗑️ Clear and start over"):
        for key in ["chunks", "pdf_name", "answer", "used_chunks"]:
...
def answer_question(question, relevant_chunks):
    ...
    system = (
        ...
        "You can also answer questions on summarizing the content of the chunks, but you cannot use any information that is not in the chunks."
